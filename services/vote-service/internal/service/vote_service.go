...
func (s *DefaultVoteService) CastVote(
	...
	if !optionValid {
		return nil, errors.NewBadRequestError(
			fmt.Sprintf(
				"Option '%s' does not exist in poll '%s'",
				optionID,
				pollID,
			),
			"INVALID_OPTION_ID",
...
func (s *DefaultVoteService) fetchPollState(
	ctx context.Context,
	pollID string,
...
func (s *DefaultVoteService) incrementPollVote(
	ctx context.Context,
