...
// AppError is the domain error struct returned by service layers.
// It keeps domain services completely decoupled from Gin/HTTP frameworks.
...
// Constructors for domain errors
...
// RespondWithError translates an error into the appropriate HTTP status code
// and sends the standardized response.APIResponse envelope.
// Internal error details are sanitized to prevent leaking stack traces or credentials.
func RespondWithError(c *gin.Context, err error) {
	...
	// Unhandled standard error -> treat as internal error without leaking details
	response.InternalError(c)
}
